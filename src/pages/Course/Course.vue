<template>
	<view class="Course">
		<view class="banner">
			<lazy-image v-if="CourseData.jsxq" :realSrc="imgUrl + CourseData.kcxq.img" placeholdSrc=""></lazy-image>
		...
		<view class="enroll">
			...
			<view class="bmzt"   v-if="CourseData.kcxq" @click="GObmzt(CourseData.kcxq.xsk_id)">
				...
				<view class="buttonActive" v-if="bmzt">{{CourseData.kcxq.yyrs}}人预约</view>
			</view>
		</view>
		<view class="content">
			...
			<view class="box1">
				<view class="v1">
					<lazy-image v-if="CourseData.jsxq" :realSrc="imgUrl2 + CourseData.jsxq.portrait" placeholdSrc=""></lazy-image>
					<view>
						<text class="t1" v-if="CourseData.jsxq">{{CourseData.jsxq.name}}</text>
						<text class="t2" v-if="CourseData.jsxq">{{CourseData.jsxq.time}}</text>						
					</view>
				</view>
				<view class="v2" @click="collectFunc" v-if="CourseData.length > 0">
	...
	export default {
		data() {
			return {
				...
				bmzt: 0,
				collect:0,
				noImg: '../../static/images/Course/bg.jpg'
			}
		},
		onLoad() {
			/* 预约精品线上课接口 */
