		...
		<view class="banner">
			...
			<view class="enroll">
				...
				<view class="bmzt" @click="GObmzt(OfflineData.kcxq.id)">
					...
					<view class="buttonActive" v-if="bmzt">{{OfflineData.count}}人报名</view>
				</view>
			</view>
		</view>
		<view class="content">
			...
			<!-- 富文本编辑 -->
			<view class="parse">
				<u-parse v-if="OfflineData.kcxq" :content="OfflineData.kcxq.content" @preview="preview" @navigate="navigate" :imgOptions="true" />
			</view>
		</view>		
<!-- 		<view class="end">
			<text>—— 我是有底线的 ——</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// },
			GObmzt () {
				console.log(this.OfflineData.bmzt)
				if(this.OfflineData.bmzt === 1) {
					this.$msg("您已经报过名了！")
				}else if(this.OfflineData.bmzt === 2) {
					this.$msg("报名成功！")
					/* 线下课详情 */
					this.$request.OfflineCourse({
						id: 1
